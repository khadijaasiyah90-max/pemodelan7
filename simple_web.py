...
def analyze_topics(file):
    ...
    try:
        ...
        try:
            ...
            coherence_text = f"⚠️ Coherence gagal: {str(e)}"
...
with gr.Blocks(title="Topic Modeling Web App") as demo:
    ...
    gr.Markdown("Upload file CSV untuk analisis topik dengan BERTopic")

    file_input = gr.File(label="Upload Dataset CSV", file_types=[".csv"])
    analyze_btn = gr.Button("🚀 Analisis Topik", variant="primary")

    summary_output = gr.Textbox(label="Ringkasan", lines=8)
    topics_output = gr.Textbox(label="Tabel Topik", lines=10)
